################################################################################################################
# music.py      Version 3.9         30-Mar-2016       Bill Manaris, Chris Benson, and Kenneth Hanson
...
class Play(jPlay):      
   ...
   def midi(material):
      ...
      if type(material) == Score:
         ...
         chordNotes = []      # used to process notes beloning in a chord
         for start, pitch, duration, velocity, channel, instrument in noteList:
            ...
            if duration == 0:   # does this note belong in a chord?
               chordNotes.append([start, pitch, duration, velocity, channel])  # add it to the list of chord notes
               ...
               chordNotes.append([start, pitch, duration, velocity, channel])

               # now, schedule all notes in the chord list using last note's duration
               for start, pitch, ignoreThisDuration, velocity, channel in chordNotes:
...
class Metronome():
   ...
   def __init__(self, tempo=60, timeSignature=[4, 4]):
      ...
      self.timer = Timer(delay, self.__callFunctions__, [], True)
